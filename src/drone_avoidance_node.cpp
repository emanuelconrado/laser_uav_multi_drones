...
/* DroneAvoidance() //{ */
DroneAvoidance::DroneAvoidance(const rclcpp::NodeOptions & options)
: rclcpp_lifecycle::LifecycleNode("manager_node", "", options)
{
  ...
  payload_state_ = nav_msgs::msg::Odometry();
...
DroneAvoidance::~DroneAvoidance() = default;
//}

/* on configure() //{ */
CallbackReturn DroneAvoidance::on_configure(const rclcpp_lifecycle::State &)
{
...
/* on activate() //{ */
CallbackReturn DroneAvoidance::on_activate([[maybe_unused]] const rclcpp_lifecycle::State & state)
{
...
/* on deactivate() //{ */
CallbackReturn DroneAvoidance::on_deactivate([[maybe_unused]] const rclcpp_lifecycle::State & state)
{
...
/* on cleanup() //{ */
CallbackReturn DroneAvoidance::on_cleanup([[maybe_unused]] const rclcpp_lifecycle::State & state)
{
...
/* on shutdown() //{ */
CallbackReturn DroneAvoidance::on_shutdown([[maybe_unused]] const rclcpp_lifecycle::State & state)
{
...
/* get parameters() //{ */
void DroneAvoidance::get_parameters()
{
...
void DroneAvoidance::configure_publishers_and_subscriptions()
{
  ...
  sub_odometry_ =
    ...
    "odometry_in", 1,
    std::bind(&DroneAvoidance::sub_odometry, this, std::placeholders::_1));

  sub_payload_odometry_ =
    create_subscription<nav_msgs::msg::Odometry>(
    "odometry_payload_in", 1,
    std::bind(&DroneAvoidance::sub_payload_odometry, this, std::placeholders::_1));

  pub_neighbor_position_velocity_ = create_publisher<laser_msgs::msg::NeighborOdomArray>(
    "neighbor_odom_out", 1);
}
//}

/* configure timers() //{ */
void DroneAvoidance::configure_timers()
{
  RCLCPP_DEBUG(get_logger(), "Configuring timers");

  timer_manager_ = create_wall_timer(
    std::chrono::duration<double>(
      1.0 / timer_manager_rate_), std::bind(&DroneAvoidance::timer_manager_callback, this), nullptr);
}
//}

/* configure clients() //{ */
void DroneAvoidance::configure_clients()
{
...
/* configure services() //{ */
void DroneAvoidance::configure_services()
{
...
/* sub odometry() //{ */
void DroneAvoidance::sub_odometry(const nav_msgs::msg::Odometry & message)
{
  if (!is_active_) {
    return;
  }
...
Eigen::Vector3d DroneAvoidance::calculateCableTension(
...
void DroneAvoidance::updateGlobalObservation()
{
  constexpr std::size_t kUavStateSize = 6;
  constexpr std::size_t kPayloadStateSize = 6;
  constexpr std::size_t kTrackingErrorSize = 3;
  ...
  global_observation_[payload_offset + 0] =
    payload_state_aux_.pose.pose.position.x / 10.0;
  global_observation_[payload_offset + 1] =
    payload_state_aux_.pose.pose.position.y / 10.0;
  global_observation_[payload_offset + 2] =
    payload_state_aux_.pose.pose.position.z / 10.0;

  global_observation_[payload_offset + 3] =
    payload_state_aux_.twist.twist.linear.x / 5.0;
  global_observation_[payload_offset + 4] =
    payload_state_aux_.twist.twist.linear.y / 5.0;
  global_observation_[payload_offset + 5] =
    payload_state_aux_.twist.twist.linear.z / 5.0;
  ...
  global_observation_[tracking_offset + 0] =
    (target_now.x() - payload_state_aux_.pose.pose.position.x) / 5.0;
  global_observation_[tracking_offset + 1] =
    (target_now.y() - payload_state_aux_.pose.pose.position.y) / 5.0;
  global_observation_[tracking_offset + 2] =
    (target_now.z() - payload_state_aux_.pose.pose.position.z) / 5.0;
  ...
  const Eigen::Vector3d payload_position(
    payload_state_aux_.pose.pose.position.x,
    payload_state_aux_.pose.pose.position.y,
    payload_state_aux_.pose.pose.position.z);

  const Eigen::Vector3d payload_velocity(
    payload_state_aux_.twist.twist.linear.x,
    payload_state_aux_.twist.twist.linear.y,
    payload_state_aux_.twist.twist.linear.z);
...
void DroneAvoidance::timer_manager_callback()
{
  if (!is_active_) {
    return;
  }
...

  {
    ...
    payload_state_aux_ = payload_state_;
...
/* // Neighbors */
/*   for (std::size_t i = 0; i < neighbors_states_aux_.size(); ++i) { */
...
/*   std::cout << "time_stamp: " << global_observation_[index++] << std::endl; */
  ...
  for (const auto & uav_name : uav_names_) {
    ...
    const auto & neighbor_state = neighbors_states_aux_[neighbor_index];

    const double latency =
      (now() - rclcpp::Time(neighbor_state.header.stamp)).seconds();
  ...
  std::sort(
    ...
    [](const auto & first_neighbor, const auto & second_neighbor) {
      const Eigen::Vector3d first_position(
        first_neighbor.pose.position.x,
        first_neighbor.pose.position.y, first_neighbor.pose.position.z);

      const Eigen::Vector3d second_position(
        second_neighbor.pose.position.x,
        second_neighbor.pose.position.y, second_neighbor.pose.position.z);
...
void DroneAvoidance::sub_payload_odometry(const nav_msgs::msg::Odometry & message)
{
  if (!first_odometry_received_) {
    first_odometry_received_ = true;
  }
  payload_state_.pose.pose.position.x = message.pose.pose.position.x;
  payload_state_.pose.pose.position.y = message.pose.pose.position.y;
  payload_state_.pose.pose.position.z = message.pose.pose.position.z;

  payload_state_.twist.twist.linear.x = message.twist.twist.linear.x;
  payload_state_.twist.twist.linear.y = message.twist.twist.linear.y;
  payload_state_.twist.twist.linear.z = message.twist.twist.linear.z;
}
//}

/* sub neighbor odometry() //{ */
void DroneAvoidance::sub_neighbor_odometry(
  ...
  const std::size_t neighbor_index)
{
  ...
  neighbors_odom_[neighbor_index] = *message;
  ...
  const Eigen::Vector3d neighbor_position(message->pose.pose.position.x,
    message->pose.pose.position.y, message->pose.pose.position.z);
  ...
  const Eigen::Vector3d relative_position = neighbor_position - this_uav_position;

  const Eigen::Vector3d relative_velocity = this_uav_velocity - neighbor_velocity;
...
RCLCPP_COMPONENTS_REGISTER_NODE(laser_uav_multi_drones::DroneAvoidance)
